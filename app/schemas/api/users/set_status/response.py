...
class SetStatusResponse(CamelModel):
    """
    Схема ответа API после обновления `bot_status` пользователя.
    ...
    platform_id: int = Field(..., description="Идентификатор пользователя в Telegram.", examples=[12312312312])
    ...
    point: int = Field(..., description="Количество поинтов пользователя.", examples=[10])
    """Количество обычных поинтов пользователя."""

    bonus: int = Field(..., description="Количество бонусных поинтов пользователя.", examples=[10])
    ...
    categories: list[CategoryResponse] = Field(..., description="Список категорий пользователя.")
