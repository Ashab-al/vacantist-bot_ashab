...
async def test_update_category(session):
    ...
    update_category_name: Category = await update_category(
        session, category.id, UpdateCategoryRequest(name=new_category_name)
    )

    find_category: Category = await session.get(Category, category.id)
...
@pytest.mark.asyncio
async def test_update_category_when_category_is_not_exist(session):
    ...
    with pytest.raises(ValueError) as excinfo:
        await update_category(
            session, category_id, UpdateCategoryRequest(name=new_category_name)
        )
