...
async def add_vacancy_to_blacklist(vacancy_id: int, db: AsyncSession) -> BlackList:
    """
    ...
    if await check_blacklist(db, vacancy):
        raise ValueError(
            f"Вакансия с контактной информацией {vacancy.contact_information} уже в черном списке"
        )
