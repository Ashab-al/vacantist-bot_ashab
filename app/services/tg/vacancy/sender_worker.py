...
async def sender_worker(
    ...
    while True:
        ...
        if vacancy is None:
            ...
            await admin_alert(bot, "Получен сигнал на завершение. Воркер останавливается.")
            queue.task_done()
            break
        ...
        async with get_async_session_for_bot() as db:
            ...
            if not users:
                ...
                await admin_alert(bot, f"Для вакансии '{vacancy.title}' не найдено подписчиков.")
                queue.task_done()
                continue 
            ...
            if blocked_user_ids:
                ...
                await admin_alert(bot, f"Обновлен статус для {len(blocked_user_ids)} заблокированных пользователей.")
