...
async def advertisement(
    ...
    Выполняет асинхронный SQL-запрос к базе данных, объединяя таблицу `Category` 
    с таблицей `subscription` по `category_id`, и подсчитывает количество пользователей 
    в каждой категории.
