...
async def reaction_choice_vacancy(
    ...
    - В зависимости от статуса вакансии:
        ...
        * Если статус `OPEN_VACANCY` — редактирует сообщение и показывает подробности вакансии с кнопками действий.

    Args:
        callback (CallbackQuery): Объект колбэка Telegram, вызванного при нажатии кнопки.
        callback_data (OpenVacancyCallback): Данные, переданные вместе с callback (в том числе ID вакансии).
...
async def reaction_choice_spam_vacancy(
    ...
    - Вызывает сервис `spam_vacancy` для обработки жалобы и возможного добавления вакансии в чёрный список.
    - Отправляет пользователю alert с результатом действия (например, "вакансия добавлена в список спама").

    Args:
        callback (CallbackQuery): Объект Telegram callback-запроса, вызванного при нажатии кнопки.
        callback_data (SpamVacancyCallback): Данные, переданные вместе с callback (включая ID вакансии).
